...
2. [Trading Strategies (PERCENTAGE, FIXED, ADAPTIVE)](#2-trading-strategies-percentage-fixed-adaptive)
3. [Multipliers (Single and Tiered)](#3-multipliers-single-and-tiered)
4. [Risk Limits](#4-risk-limits)
5. [Core Strategy Components](#5-core-strategy-components)
6. [Strategy Parameters Summary](#6-strategy-parameters-summary)
7. [Detailed Strategy Logic Deep Dive](#7-detailed-strategy-logic-deep-dive)
8. [Strategy Rationale](#8-strategy-rationale)
9. [Complete Strategy Logic Flow (Pseudo-Code)](#9-complete-strategy-logic-flow-pseudo-code)
10. [Performance Considerations](#10-performance-considerations)
11. [Strategy Limitations](#11-strategy-limitations)
12. [Optimization Tips](#12-optimization-tips)
13. [Strategy Monitoring and Metrics](#13-strategy-monitoring-and-metrics)
14. [Strategy Tuning Guide](#14-strategy-tuning-guide)
15. [Future Enhancements (Potential)](#15-future-enhancements-potential)
...
|-----------|----------|---------------|
| **Copy Strategy** | PERCENTAGE, FIXED, or ADAPTIVE | PERCENTAGE (10%) |
| **Position Sizing** | Configurable % or fixed $ amount | 10% of whale size (PERCENTAGE) |
| **Multipliers** | Single + optional tiered multipliers | 1.0x (single), none (tiered) |
...
| **Risk Limits** | Max/min order, max position, max daily | Configurable per strategy |
...
| **Minimum Trade** | Skip trades below threshold | Configurable (default: $1) |
...
**Key Tenets:**
1. **Scale Down:** Never risk more than you can afford (configurable % or fixed amount)
...
6. **Adapt:** Adjust position size based on trade characteristics (ADAPTIVE strategy)

---

## 2. Trading Strategies (PERCENTAGE, FIXED, ADAPTIVE)

The bot supports three distinct trading strategies for calculating order sizes. Each strategy has different use cases and risk profiles.

### 2.1 PERCENTAGE Strategy

**How it works:** Copies a fixed percentage of the trader's order size.

**Configuration:**
- `COPY_STRATEGY=PERCENTAGE`
- `COPY_SIZE=10.0` (percentage value, e.g., 10.0 = 10%)

**Calculation:**
```
Your Order Size (USD) = Trader Order Size (USD) × (COPY_SIZE / 100)
```

**Example 1: Small Trade**
```
Trader buys: 100 shares @ $0.50 = $50
COPY_SIZE = 10.0%
Your order: $50 × 0.10 = $5.00
Result: You buy 10 shares @ $0.50
```

**Example 2: Large Trade**
```
Trader buys: 5,000 shares @ $0.75 = $3,750
COPY_SIZE = 10.0%
Your order: $3,750 × 0.10 = $375.00
Result: You buy 500 shares @ $0.75
```

**Example 3: With Multiplier**
```
Trader buys: $1,000 worth
COPY_SIZE = 10.0%
TRADE_MULTIPLIER = 2.0
Base order: $1,000 × 0.10 = $100
After multiplier: $100 × 2.0 = $200
Result: You buy $200 worth
```

**Best for:**
- Consistent risk exposure relative to trader
- Following traders with varying trade sizes
- Maintaining proportional position sizes

**Pros:**
- Automatically scales with trader's conviction
- Simple to understand and configure
- Works well for all trade sizes

**Cons:**
- Can result in very large orders if trader makes huge trades
- Requires MAX_ORDER_SIZE_USD cap for safety

---

### 2.2 FIXED Strategy

**How it works:** Always uses a fixed dollar amount per trade, regardless of trader's order size.

**Configuration:**
- `COPY_STRATEGY=FIXED`
- `COPY_SIZE=50.0` (dollar amount, e.g., 50.0 = $50)

**Calculation:**
```
Your Order Size (USD) = COPY_SIZE (always the same)
```

**Example 1: Small Trader Trade**
```
Trader buys: $20 worth
COPY_SIZE = $50.0
Your order: $50.00 (ignores trader's $20)
Result: You buy $50 worth (more than trader!)
```

**Example 2: Large Trader Trade**
```
Trader buys: $10,000 worth
COPY_SIZE = $50.0
Your order: $50.00 (ignores trader's $10,000)
Result: You buy $50 worth (much less than trader)
```

**Example 3: With Multiplier**
```
Trader buys: $5,000 worth
COPY_SIZE = $50.0
TRADE_MULTIPLIER = 1.5
Base order: $50.00
After multiplier: $50 × 1.5 = $75.00
Result: You buy $75 worth
```

**Best for:**
- Fixed risk per trade
- Budget-conscious trading
- Consistent position sizes regardless of trader activity

**Pros:**
- Predictable capital usage
- Easy to budget
- No surprise large orders

**Cons:**
- Doesn't scale with trader's conviction
- May miss opportunities on large trades
- May over-trade on small trader positions

---

### 2.3 ADAPTIVE Strategy

**How it works:** Dynamically adjusts the percentage based on the trader's order size. Uses higher percentage for small trades, lower percentage for large trades.

**Configuration:**
- `COPY_STRATEGY=ADAPTIVE`
- `COPY_SIZE=10.0` (base percentage at threshold)
- `ADAPTIVE_MIN_PERCENT=5.0` (minimum percentage for very large trades)
- `ADAPTIVE_MAX_PERCENT=15.0` (maximum percentage for very small trades)
- `ADAPTIVE_THRESHOLD_USD=500.0` (threshold where behavior changes)

**Calculation Logic:**

For trades **below** the threshold:
```
Effective % = MAX_PERCENT - (MAX_PERCENT - BASE_PERCENT) × (trade_size / threshold)
```

For trades **above** the threshold:
```
Effective % = BASE_PERCENT - (BASE_PERCENT - MIN_PERCENT) × min(1, (trade_size / threshold - 1))
```

**Example 1: Very Small Trade (Below Threshold)**
```
Trader buys: $100 worth
Threshold: $500
MAX_PERCENT = 15.0%
BASE_PERCENT = 10.0%

Factor = $100 / $500 = 0.2
Effective % = 15.0 - (15.0 - 10.0) × 0.2 = 15.0 - 1.0 = 14.0%

Your order: $100 × 0.14 = $14.00
Result: You use 14% (higher than base 10%)
```

**Example 2: At Threshold**
```
Trader buys: $500 worth
Threshold: $500
Effective % = 10.0% (exactly BASE_PERCENT)

Your order: $500 × 0.10 = $50.00
Result: You use exactly 10%
```

**Example 3: Large Trade (Above Threshold)**
```
Trader buys: $2,000 worth
Threshold: $500
MIN_PERCENT = 5.0%
BASE_PERCENT = 10.0%

Factor = min(1, ($2,000 / $500 - 1)) = min(1, 3.0) = 1.0
Effective % = 10.0 - (10.0 - 5.0) × 1.0 = 10.0 - 5.0 = 5.0%

Your order: $2,000 × 0.05 = $100.00
Result: You use 5% (lower than base 10%)
```

**Example 4: Medium Trade (Above Threshold, Partial)**
```
Trader buys: $750 worth
Threshold: $500
MIN_PERCENT = 5.0%
BASE_PERCENT = 10.0%

Factor = min(1, ($750 / $500 - 1)) = min(1, 0.5) = 0.5
Effective % = 10.0 - (10.0 - 5.0) × 0.5 = 10.0 - 2.5 = 7.5%

Your order: $750 × 0.075 = $56.25
Result: You use 7.5% (between base and min)
```
...
Effective Percentage
    |
15% |●─────────────────┐  (MAX_PERCENT for very small trades)
    |                   │
10% |                   ●─── (BASE_PERCENT at threshold)
    |                   │
 5% |                   └───● (MIN_PERCENT for very large trades)
    |_______________________|_________________> Trader Order Size
     $0                  $500              $2000+
                    (threshold)
```

**Best for:**
- Balancing risk across different trade sizes
- Being more aggressive on small trades, conservative on large trades
- Following traders with highly variable trade sizes

**Pros:**
- Automatically adjusts risk based on trade size
- More exposure on small trades (higher conviction signal)
- Less exposure on large trades (risk management)

**Cons:**
- More complex to configure
- Requires tuning of threshold and min/max percentages
- May need adjustment based on trader behavior

---

## 3. Multipliers (Single and Tiered)

Multipliers allow you to scale your base order size calculated by the strategy. You can use a single multiplier for all trades, or tiered multipliers that vary based on trade size.

### 3.1 Single Multiplier

**Configuration:**
- `TRADE_MULTIPLIER=1.5` (multiplies all orders by 1.5x)

**How it works:**
```
Final Order = Base Order (from strategy) × TRADE_MULTIPLIER
```
...
Result: You buy $150 worth (15% effective)
```

**Example with FIXED Strategy:**
```
Trader buys: $5,000 worth
COPY_SIZE = $50.0
TRADE_MULTIPLIER = 2.0
...
Base order: $50.00
After multiplier: $50 × 2.0 = $100
Result: You buy $100 worth
```

**Use cases:**
- Aggressive trading: `TRADE_MULTIPLIER=2.0` (2x exposure)
- Conservative trading: `TRADE_MULTIPLIER=0.5` (half exposure)
- Normal trading: `TRADE_MULTIPLIER=1.0` (no change)

---

### 3.2 Tiered Multipliers

**Configuration:**
- `TIERED_MULTIPLIERS="1-10:2.0,10-100:1.0,100-500:0.5,500+:0.2"`

**Format:** `"range:multiplier,range:multiplier,..."`
- Ranges: `"min-max"` or `"min+"` for open-ended
- Multipliers: Decimal numbers (e.g., `2.0`, `0.5`)

**How it works:**
1. Calculate base order using strategy
2. Determine which tier the trader's order size falls into
3. Apply the tier's multiplier
4. If no tier matches, no tiered multiplier is applied
...
**Example 1: Small Trade ($5)**
```
Trader buys: $5 worth
COPY_SIZE = 10.0% (PERCENTAGE)
Base order: $5 × 0.10 = $0.50
Tier: "1-10" → multiplier = 2.0
After tiered multiplier: $0.50 × 2.0 = $1.00
Result: You buy $1 worth (aggressive on small trades)
```

**Example 2: Medium Trade ($50)**
```
Trader buys: $50 worth
COPY_SIZE = 10.0%
Base order: $50 × 0.10 = $5.00
Tier: "10-100" → multiplier = 1.0
After tiered multiplier: $5.00 × 1.0 = $5.00
Result: You buy $5 worth (normal scaling)
```

**Example 3: Large Trade ($200)**
```
Trader buys: $200 worth
COPY_SIZE = 10.0%
Base order: $200 × 0.10 = $20.00
Tier: "100-500" → multiplier = 0.5
After tiered multiplier: $20.00 × 0.5 = $10.00
Result: You buy $10 worth (conservative on large trades)
```

**Example 4: Very Large Trade ($1,000)**
```
Trader buys: $1,000 worth
COPY_SIZE = 10.0%
Base order: $1,000 × 0.10 = $100.00
Tier: "500+" → multiplier = 0.2
After tiered multiplier: $100.00 × 0.2 = $20.00
Result: You buy $20 worth (very conservative on huge trades)
```

**Combining Single and Tiered Multipliers:**
```
Trader buys: $100 worth
COPY_SIZE = 10.0%
TRADE_MULTIPLIER = 1.5
TIERED_MULTIPLIERS="100-500:0.5"

Step 1 (strategy): $100 × 0.10 = $10.00
Step 2 (single): $10.00 × 1.5 = $15.00
Step 3 (tiered): $15.00 × 0.5 = $7.50
Final order: $7.50
```

**Use cases:**
- **Aggressive on small, conservative on large:**
  ```
  TIERED_MULTIPLIERS="1-50:2.0,50-200:1.0,200+:0.5"
  ```
- **Very conservative on large trades:**
  ```
  TIERED_MULTIPLIERS="1-100:1.0,100-500:0.5,500+:0.1"
  ```
- **Only scale small trades:**
  ```
  TIERED_MULTIPLIERS="1-20:3.0"
  ```
...
Risk limits provide hard caps to protect your capital regardless of strategy or multiplier settings.

### 4.1 Maximum Order Size

**Configuration:**
- `MAX_ORDER_SIZE_USD=100.0` (maximum USD per single order)

**How it works:**
```
Final Order = min(Calculated Order, MAX_ORDER_SIZE_USD)
```
...
Calculated: $10,000 × 0.10 × 2.0 = $2,000
Capped at: min($2,000, $100) = $100.00
Result: You buy $100 worth (capped for safety)
```

**Use case:** Prevents unexpectedly large orders from any strategy or multiplier combination.

---

### 4.2 Minimum Order Size

**Configuration:**
- `MIN_ORDER_SIZE_USD=1.0` (minimum USD per single order)

**How it works:**
```
if Final Order < MIN_ORDER_SIZE_USD:
    Skip the trade
```
...
Calculated: $5 × 0.10 = $0.50
Since $0.50 < $1.0: Trade is SKIPPED
```

**Use case:** Prevents dust orders and trades that aren't worth the gas fees.

---

### 4.3 Maximum Position Size

**Configuration:**
- `MAX_POSITION_SIZE_USD=500.0` (optional, maximum total position per market)

**How it works:**
```
Current Position + New Order <= MAX_POSITION_SIZE_USD
```

**Example:**
```
Current position in Market A: $400
New order calculated: $150
MAX_POSITION_SIZE_USD = $500.0

Check: $400 + $150 = $550 > $500
Result: Order reduced to $100 (or skipped)
```

**Use case:** Limits exposure to any single market, preventing over-concentration.

---

### 4.4 Maximum Daily Volume

**Configuration:**
- `MAX_DAILY_VOLUME_USD=1000.0` (optional, maximum total trading volume per day)

**How it works:**
```
Daily Volume + New Order <= MAX_DAILY_VOLUME_USD
```

**Example:**
```
Trades today: $850
New order calculated: $200
MAX_DAILY_VOLUME_USD = $1000.0

Check: $850 + $200 = $1050 > $1000
Result: Order reduced to $150 (or skipped)
```

**Use case:** Limits total trading activity per day, useful for budget management.
