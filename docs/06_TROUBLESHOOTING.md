...
1. **Insufficient funds:**
   - Check funder wallet has enough USDC/USDC.e
   - Check gas (MATIC) for fees
   - Minimum recommended: $50-100 USDC

2. **Missing token approvals:**
   - **For buy orders:** USDC must be approved for exchange contracts
   - **For sell orders:** Conditional Tokens must be approved for exchange contracts
   - **Fix:** Run `cargo run --release wallet check-allowance`
   - **For Gnosis Safe:** Must approve through Safe interface (see approve_tokens output)
...
**Problem:** Order submission fails with `{"error":"invalid signature"}`.

**Solutions:**

1. **Check PRIVATE_KEY format:**
   - Must be 64 hex characters (no `0x` prefix)
   - Verify no extra spaces or characters

2. **For Gnosis Safe wallets:**
   - Verify `PRIVATE_KEY` belongs to an authorized signer of the Safe
   - Verify `FUNDER_ADDRESS` is the correct Gnosis Safe address (proxy contract)
   - Ensure the signer has permission to execute transactions from the Safe
   - Check Safe threshold settings (must meet minimum signatures required)

3. **For regular EOA wallets:**
   - Ensure `PRIVATE_KEY` matches the wallet at `FUNDER_ADDRESS`
   - If addresses don't match, use matching private key and address
...
### "not enough balance / allowance" for sell orders

**Problem:** Buy orders work, but sell orders fail with "not enough balance / allowance".

**Cause:** Conditional Tokens (ERC-1155) are not approved for the exchange contract.
...
2. **For Gnosis Safe:**
   - Follow manual approval steps shown by the script
   - Go to https://app.safe.global/ → Your Safe → Transaction Builder
   - Create transaction:
     - **Contract:** `0x4d97dcd97ec945f40cf65f87097ace5ea0476045` (Conditional Tokens)
...
3. **For EOA wallets:**
   - The `approve_tokens` script will auto-approve Conditional Tokens
   - Invalid token ID
